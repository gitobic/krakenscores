  ...
  return (
    <section className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-bold text-gray-900">Tournament-day schedule adjustments</h2>
          <p className="mt-1 text-sm text-gray-700">Admin only. Preview moves and conflicts before anything is saved.</p>
        </div>
        <div className="flex rounded-md border border-gray-300 bg-white p-1">
          <button type="button" onClick={() => { setMode('move'); setPreview([]) }} className={`rounded px-3 py-2 text-sm font-semibold ${mode === 'move' ? 'bg-blue-700 text-white' : 'text-gray-700'}`}>Move one game</button>
          <button type="button" onClick={() => { setMode('shift'); setPreview([]) }} className={`rounded px-3 py-2 text-sm font-semibold ${mode === 'shift' ? 'bg-blue-700 text-white' : 'text-gray-700'}`}>Shift later games</button>
        </div>
      </div>

      <div className="mt-4 grid gap-3 md:grid-cols-4">
        <label className="text-sm font-semibold text-gray-800 md:col-span-2">Game
          <select value={matchId} onChange={event => selectMatch(event.target.value)} className="mt-1 w-full rounded border border-gray-300 bg-white p-2">
        ...
        {mode === 'move' ? <>
          <label className="text-sm font-semibold text-gray-800">New date<input type="date" value={date} onChange={event => { setDate(event.target.value); setPreview([]) }} className="mt-1 w-full rounded border border-gray-300 bg-white p-2" /></label>
          <label className="text-sm font-semibold text-gray-800">New time<input type="time" value={time} onChange={event => { setTime(event.target.value); setPreview([]) }} className="mt-1 w-full rounded border border-gray-300 bg-white p-2" /></label>
          <label className="text-sm font-semibold text-gray-800 md:col-start-3">New pool<select value={poolId} onChange={event => { setPoolId(event.target.value); setPreview([]) }} className="mt-1 w-full rounded border border-gray-300 bg-white p-2">{pools.map(pool => <option key={pool.id} value={pool.id}>{pool.name}</option>)}</select></label>
        </> : <label className="text-sm font-semibold text-gray-800">Delay in minutes<input type="number" min="-180" max="360" step="5" value={shiftMinutes} onChange={event => { setShiftMinutes(Number(event.target.value)); setPreview([]) }} className="mt-1 w-full rounded border border-gray-300 bg-white p-2" /><span className="mt-1 block text-xs font-normal text-gray-600">Negative values move games earlier.</span></label>}
      </div>

      <button type="button" disabled={!selected || (mode === 'move' && (!date || !time || !poolId)) || (mode === 'shift' && shiftMinutes === 0)} onClick={buildPreview} className="mt-4 rounded bg-gray-900 px-4 py-2 font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40">Preview change</button>

      {preview.length > 0 && <div className="mt-4 rounded border border-gray-300 bg-white p-4">
        <h3 className="font-bold text-gray-900">Preview: {preview.length} affected game{preview.length === 1 ? '' : 's'}</h3>
        <ul className="mt-2 max-h-48 space-y-1 overflow-auto text-sm text-gray-700">
          {preview.map(change => {
