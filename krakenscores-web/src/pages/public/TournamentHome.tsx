...
function MatchCard({ item, favoriteIds }: { item: HomeMatch; favoriteIds: Set<string> }) {
  ...
  return <article className={`overflow-hidden rounded-xl border bg-white shadow-sm dark:bg-slate-900 ${isFavorite ? 'border-blue-500 ring-2 ring-blue-100 dark:ring-blue-950' : 'border-slate-200 dark:border-slate-700'}`}>
    ...
    <div className="p-4">
      ...
      <div className="mt-3 grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-2">
        <div className="font-bold text-slate-950 dark:text-white">{match.darkTeamLabel || teamPublicName(darkTeam, darkClub)}</div><div className="text-2xl font-black tabular-nums text-slate-950 dark:text-white">{final || match.status === 'in_progress' ? match.darkTeamScore ?? '–' : ''}</div>
        <div className="font-bold text-slate-950 dark:text-white">{match.lightTeamLabel || teamPublicName(lightTeam, lightClub)}</div><div className="text-2xl font-black tabular-nums text-slate-950 dark:text-white">{final || match.status === 'in_progress' ? match.lightTeamScore ?? '–' : ''}</div>
      </div>
...
function MatchSection({ title, accent, empty, items, favoriteIds }: { title: string; accent: string; empty: string; items: HomeMatch[]; favoriteIds: Set<string> }) {
  return <section className="mt-8">
    <div className="mb-3 flex items-center gap-3"><span className={`h-3 w-3 rounded-full ${accent}`} /><h2 className="text-xl font-black text-slate-950 dark:text-white">{title}</h2><span className="rounded-full bg-slate-200 px-2 py-0.5 text-xs font-bold text-slate-700 dark:bg-slate-700 dark:text-slate-200">{items.length}</span></div>
    {items.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map(item => <MatchCard key={item.match.id} item={item} favoriteIds={favoriteIds} />)}</div> : <div className="rounded-xl border border-dashed border-slate-300 bg-white p-6 text-slate-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">{empty}</div>}
  </section>
}
...
export default function TournamentHome() {
  ...
  return <div className="min-h-screen bg-slate-100 pb-16 dark:bg-slate-950"><PublicNav />
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <section className={`relative z-10 ${teamFinderSpacing(activeAnnouncements.length > 0)} rounded-2xl border border-slate-200 bg-white p-5 shadow-lg dark:border-slate-700 dark:bg-slate-900`}>
        <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-black text-slate-950 dark:text-white">Find your team</h2><p className="text-sm text-slate-600 dark:text-slate-300">Search once, then tap the star to keep it on this device—no account needed.</p></div>{favoriteIds.size > 0 && <button type="button" onClick={() => { setFavoritesOnly(value => !value); setSelectedTeamId('') }} className={`rounded-full px-4 py-2 text-sm font-bold ${favoritesOnly ? 'bg-blue-700 text-white' : 'bg-blue-50 text-blue-800 dark:bg-blue-950 dark:text-blue-200'}`}>★ My teams ({favoriteIds.size})</button>}</div>
        <div className="relative mt-4"><input aria-label="Search teams" value={query} onFocus={() => setSearchOpen(true)} onChange={event => { setQuery(event.target.value); setSearchOpen(true); setSelectedTeamId(''); setFavoritesOnly(false) }} placeholder="Team name, club, or abbreviation…" className="w-full rounded-xl border-2 border-slate-300 bg-white px-4 py-3 text-base text-slate-950 outline-none focus:border-blue-600 dark:border-slate-600 dark:bg-slate-800 dark:text-white" />
          {query && searchOpen && <div className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">{searchResults.length ? searchResults.map(team => { const club = clubById.get(team.clubId); const division = divisionById.get(team.divisionId); return <div key={team.id} className="flex items-center border-b border-slate-100 last:border-0 dark:border-slate-700"><button type="button" onClick={() => { setSelectedTeamId(team.id); setQuery(team.name); setSearchOpen(false) }} className="min-w-0 flex-1 p-3 text-left hover:bg-slate-50 dark:hover:bg-slate-800"><span className="block truncate font-bold text-slate-950 dark:text-white">{team.name}</span><span className="mt-1 flex flex-wrap items-center gap-2 text-sm text-slate-500 dark:text-slate-400"><span>{club?.name || 'Independent team'}</span><span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2 py-0.5 font-bold text-slate-700 dark:bg-slate-700 dark:text-slate-100"><span className="h-2 w-2 rounded-full" style={{ backgroundColor: division?.colorHex || '#64748b' }} />{division?.name || 'Division TBD'}</span></span></button><button type="button" aria-label={`${favoriteIds.has(team.id) ? 'Remove' : 'Add'} ${team.name} ${favoriteIds.has(team.id) ? 'from' : 'to'} favorites`} onClick={() => toggleFavorite(team.id)} className="p-4 text-2xl text-blue-700 dark:text-blue-300">{favoriteIds.has(team.id) ? '★' : '☆'}</button></div> }) : <div className="p-4 text-slate-600 dark:text-slate-300">No matching teams.</div>}</div>}
        </div>
